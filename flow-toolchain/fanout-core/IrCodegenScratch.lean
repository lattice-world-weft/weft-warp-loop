...
--
-- This script writes IrCodegenScratch_out.s/.o (both gitignored
-- intermediates, regenerated by every run) and assembles the .o via
-- riscv-none-elf-as. IrCodegenScratch_out.elf - the checkpoint 4 test
-- target actually loads - is a further, separate link step this script
-- does NOT run automatically (checkpoint 3's own proof only needed the
-- assembler, not a linker), matching s7_guest.elf's own precedent of
-- being a committed, not build-generated, artifact:
--
--   riscv-none-elf-gcc -march=rv64gc -mabi=lp64d -static -nostdlib \
--     -nostartfiles -Wl,-e,Fanoutcore_hysteresisTicksFor \
--     IrCodegenScratch_out.o -o IrCodegenScratch_out.elf
--
-- (-Wl,-e,<symbol> points the ELF's own entry point directly at the
-- compiled function, avoiding the need for a _start stub - vmcall()
-- never runs from e_entry, but the linker still requires a valid one to
-- produce an EXEC, not just a relocatable object.)
--
